...
{
	assert(opts && isObject(opts), 'you must pass an options object');
	assert(opts.hosts && isArray(opts.hosts), 'you must pass an array in the `hosts` option');
	assert(opts.username && isString(opts.username), 'you must pass a `username` option');
	assert(opts.password && isString(opts.password), 'you must pass a `password` option');
	assert(opts.database && isString(opts.database), 'you must pass a `database` option');
...
InfluxOutput.prototype.toString = function toString()
{
	return '[ InfluxDB ' + this.options.database + ' @ ' +
		map(this.options.hosts, function(h) { return h.host; }).join(', ') +
		' ]';
};

InfluxOutput.prototype._write = function _write(event, encoding, callback)
{
	var point = pickBy(event, function(v) { return !isObject(v) && !isArray(v); });
	if (event.time)
