...
var GraphiteUDP = module.exports = function GraphiteUDP(opts)
{
	...
	assert(opts.host && isString(opts.host), 'you must pass a hostname in the `host` option');
	assert(opts.port && isNumber(opts.port), 'you must pass a `port` option');

	stream.Writable.call(this, { objectMode: true });
	if (!opts.batchDelay || !isNumber(opts.batchDelay)) opts.batchDelay = 5000; // in ms
...
GraphiteUDP.prototype._write = function _write(event, encoding, callback)
{
	...
	var ts = event.time;
	if (!ts) ts = new Date();
	if (isObject(ts)) ts = Math.round(ts.getTime() / 1000);
