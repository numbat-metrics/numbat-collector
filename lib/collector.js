...
var Collector = module.exports = function Collector(opts)
{
    ...
    assert(opts.incoming && _.isObject(opts.incoming), 'you must pass a host/port or path object in `incoming`');
...
Collector.prototype.listen = function listen()
{
    ...
    if (opts.incoming.path)
    {
        fs.unlink(opts.incoming.path, function()
        {
            this.collectServer.listen(opts.incoming.path, report.bind(this, opts.incoming.path));
        }.bind(this));
    }
    else
        this.collectServer.listen(opts.incoming.port, opts.incoming.host, report.bind(this, opts.incoming.port));
};
