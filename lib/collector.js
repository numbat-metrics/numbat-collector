...
var Collector = module.exports = function Collector(opts)
{
    ...
    this.adminServer = this.intializeAdminServer(restifyOpts);
...
{
    this.adminServer.listen(this.admin.port, function()
    {
        this.log.info('admin API now listening on port ' + this.admin.port);
    }.bind(this));
    this.collectServer.listen(this.port, function()
    {
        this.log.info('metrics collector now listening on port ' + this.metrics.port);
    }.bind(this));
};
...
Collector.prototype.initializeAdminServer = function initializeAdminServer(restifyOpts)
{
